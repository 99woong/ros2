...
inline uint16_t calc_col(float x, float y)
{
    ...
    return static_cast<uint16_t>(
        static_cast<int>(std::round(azi * N_COLS / 360.0)) % N_COLS);
...
public:
    OS1BridgeNode() : Node("os1_sim_bridge")
    {
        ...
        this->declare_parameter<std::string>("frame_id",     "sim_lidar");
...
private:
    ...
    void build_output_template()
    {
        ...
        // PF:: 상수 대신 실제 값을 직접 지정 (컴파일러/버전별 상수값 불일치 방지)
        ...
        tmpl_.fields = {
            ...
            f("y",            OFF_Y,            7),  // FLOAT32
            f("z",            OFF_Z,            7),  // FLOAT32
            f("intensity",    OFF_INTENSITY,    7),  // FLOAT32, offset=16
            f("t",            OFF_T,            6),  // UINT32,  offset=20 ← 실측 datatype:6
            f("reflectivity", OFF_REFLECTIVITY, 4),  // UINT16,  offset=24
            f("ring",         OFF_RING,         4),  // UINT16,  offset=26
            f("ambient",      OFF_AMBIENT,      4),  // UINT16,  offset=28
            f("range",        OFF_RANGE,        6),  // UINT32,  offset=32 ← 실측 datatype:6
        };
    }
    ...
    void on_cloud(const sensor_msgs::msg::PointCloud2::SharedPtr in)
    {
        ...
        if (written == 0) 
        {
            RCLCPP_WARN_THROTTLE(get_logger(), *get_clock(), 3000,
                "빈 프레임 (written=0). Isaac Sim 플레이 중인지 확인.");
            return;
        }
