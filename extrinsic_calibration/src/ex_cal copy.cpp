...
// [수정된 부분]: 스케일 팩터 S를 1.0으로 강제할지 여부를 결정하는 플래그
// 1로 설정하면 S=1.0 (Rigid Body Transform)으로 계산됩니다.
#define FORCE_SCALE_ONE 0
// #define FORCE_SCALE_ONE 1
...
// constexpr double GLS_POSITION_CORRECTION_FACTOR = 0.79; 
...
std::vector<Measurement> read_csv(const std::string& filepath) {
    ...
    while (std::getline(file, line)) {
        ...
        if (values.size() >= 6) {
            ...
            m.gls_x = values[3] * GLS_POSITION_CORRECTION_FACTOR; // (GLS Y) * 1.0 -> X'
            
            // GLS X (Left) -> VSLAM Y (Left)
            // [부호 수정 유지]
            // m.gls_y = -values[3] * GLS_POSITION_CORRECTION_FACTOR; // (GLS X) * 1.0 -> Y'
            m.gls_y = values[4] * GLS_POSITION_CORRECTION_FACTOR; // (GLS X) * 1.0 -> Y'
            
            // 2-2. 헤딩 변환 (축 정렬 + 회전 규약 변환)
            double gls_heading_rad = degrees_to_radians(values[5]); // Degree -> Radian 변환 적용

            // m.gls_heading = normalize_angle_to_pi_range(PI - gls_heading_rad); 
            m.gls_heading = normalize_angle_to_pi_range(-gls_heading_rad); 
            // =======================================================================
...
// === 1단계: 외부 보정 파라미터 계산 (Scale Factor S 계산 여부 선택) ===
ExtrinsicParams calculate_extrinsic_params(const std::vector<Measurement>& data) {
    ...
    // =========================================================================
    // 3. 스케일 팩터 (S) 및 평행 이동 오프셋 (Delta X, Delta Y) 계산
    // =========================================================================

#if FORCE_SCALE_ONE // S=1.0 강제 적용 (Rigid Body Transform)
    
    // [Rigid Body Transform]: S = 1.0 고정
    params.scale_factor = 0.79; 
    
    // T = V_centroid - R * G_centroid
    
    // GLS 무게 중심 좌표에 (Rotation)만 적용
    double gls_rotated_x = gls_centroid_x * cos_dh - gls_centroid_y * sin_dh;
    double gls_rotated_y = gls_centroid_x * sin_dh + gls_centroid_y * cos_dh;

    // VSLAM 무게 중심과의 차이
    params.delta_x = vslam_centroid_x - gls_rotated_x;
    params.delta_y = vslam_centroid_y - gls_rotated_y;

#else // Similarity Transform: 스케일 팩터 S 계산
...
// (이 함수는 S가 1.0이든, 계산된 값이든 params에 있는 값을 사용하여 평가합니다.)
EvaluationResult evaluate_calibration(std::vector<Measurement>& data, const ExtrinsicParams& params) {
    ...
    double cos_dh = std::cos(params.delta_heading);
    double sin_dh = std::sin(params.delta_heading);
    double scale = params.scale_factor; // 계산된 스케일 팩터 사용 (S=1.0 또는 최적의 S)
...
int main(int argc, char** argv) 
{
    ...
    // 이상치 제거 기준 1.5x 유지
    const double OUTLIER_THRESHOLD_MULTIPLIER = 1.5; 
...
#if FORCE_SCALE_ONE
    std::cout << "(스케일 팩터 S=1.0 강제 적용 - Rigid Body Transform)" << std::endl;
#else
    std::cout << "(스케일 팩터 S 추정 - Similarity Transform)" << std::endl;
#endif
    std::cout << "(" << OUTLIER_THRESHOLD_MULTIPLIER << " x P_RMSE 및 R_RMSE를 초과하는 데이터 포인트를 제거)" << std::endl;
    std::cout << "================================================" << std::endl;
    ...
    for (int i = 0; i < MAX_ITERATIONS; ++i) {
        ...
        // 1. 현재 데이터셋으로 파라미터 계산 (S=1.0 또는 S 추정)
...
#if FORCE_SCALE_ONE
    std::cout << "스케일 팩터 (Scale Factor, S):       " << params.scale_factor << " (강제 적용)" << std::endl; 
#else
    std::cout << "스케일 팩터 (Scale Factor, S):       " << params.scale_factor << " (계산된 값)" << std::endl; 
#endif
