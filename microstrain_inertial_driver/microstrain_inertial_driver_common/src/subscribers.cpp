...
{
  printf("Subscribers node_: %p\n", node_);
  printf("Subscribers ACTIVATE: %p\n", this);
...
//     external_time_sub_ = node_->create_subscription<sensor_msgs::msg::TimeReference>(
//     "/ext/time",
//     rclcpp::QoS(10),
//     std::bind(&Subscribers::externalTimeCallback, this, std::placeholders::_1)
// );  
  }
...
void Subscribers::externalTimeCallback(const TimeReferenceMsg& time)
{
  ...
  std::cout << gps_week_number << " " << gps_tow << std::endl;
