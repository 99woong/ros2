...
public:
    VSLAMReceiver() : Node("vslam_receiver"), sockfd(-1), running(false), packet_count(0)
    {
        ...
        max_path_length = this->get_parameter("max_path_length").as_int();
        ...
        RCLCPP_INFO(this->get_logger(), "VSLAM Receiver Node 생성 완료");
        RCLCPP_INFO(this->get_logger(), "Max path length: %zu", max_path_length);
    }
...
private:
    ...
    void processPacket(uint8_t* buffer)
    {
        ...
        // 최대 길이 제한
        if (path_msg.poses.size() > max_path_length)
        {
        ...
        if (++log_count % 10 == 0)  // 10개당 1번만 출력
        {
            RCLCPP_INFO(this->get_logger(),
                "수신: pos(%.4f, %.4f, %.4f) m, yaw: %.2f deg, inlier: %d%%, path_size: %zu",
