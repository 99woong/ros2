                             ...
                             rclcpp::Node::SharedPtr node)
{
    ...
    for (const auto& edge : j["edges"])
    {
        ...
        if (node_map.find(start_id) == node_map.end() || 
            // node_map.find(ceter_id) == node_map.end() ||
...
public:
    ...
    void message_arrived(mqtt::const_message_ptr msg) override
    {
        ...
        {
            // std::cout << "recv visualzation : " << i  << " " << payload_ <<  std::endl;
...
std::string getCurrentTimestampISO8601() {
    // 예시: 실제 ISO 8601 시각 생성 로직이 필요합니다.
    return "2025-11-04T05:57:00.00Z"; 
}

// 팩트시트 요청 ID를 관리하기 위한 정적 변수 (혹은 전역 변수)
static uint32_t factsheet_request_id = 0; 
// const int QOS = 1; // QOS는 상수로 이미 정의되어 있다고 가정합니다.
...
int main(int argc, char* argv[])
{
    ...
    try
    {
        ...
        //     // JSON payload 생성
        //     nlohmann::json instant_action_json;
        //     instant_action_json["actionType"] = "factsheetRequest";

        //     // JSON 문자열 직렬화
        //     std::string payload_req_factsheet = instant_action_json.dump();
        ...
        if(1)
        {
            std::cout << "Publishing factsheet request instantAction message to: " << FACTS_TOPIC << std::endl;

            // ⭐ 1. VDA5050 2.1 스키마에 맞는 JSON payload 생성
            ...
            // FMS의 정보로 채우는 것이 원칙이지만, AGV 정보를 재사용하는 경우도 많습니다.
            ...
            // --- 필수 Action 객체 생성 및 추가 ---
            nlohmann::json factsheet_request_action;
            
            factsheet_request_action["actionType"] = "factsheetRequest"; 
            factsheet_request_action["actionId"] = "FMS_FS_REQ_" + std::to_string(factsheet_request_id); // 고유한 ID
            factsheet_request_action["blockingType"] = "NONE"; // 필수 (NONE, SOFT, HARD 중 선택)
            
            // actions 배열에 액션 객체 추가
            instant_action_msg["actions"].push_back(factsheet_request_action);


            // JSON 문자열 직렬화
            std::string payload_req_factsheet = instant_action_msg.dump(4); // 4는 가독성을 위한 들여쓰기

            // MQTT 발행
            auto instant_msg = mqtt::make_message(FACTS_TOPIC, payload_req_factsheet);
            instant_msg->set_qos(QOS);
            client.publish(instant_msg)->wait(); // 실제 발행 로직은 그대로 유지
            
            std::cout << "Factsheet request message published!" << std::endl;
            std::cout << "Payload: \n" << payload_req_factsheet << std::endl;
        }
