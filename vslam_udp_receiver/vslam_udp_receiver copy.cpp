...
struct VSLAMData {
    ...
    int64_t mmx;           // unit : 0.001 [m]
    int64_t mmy;           // unit : 0.001 [m]
    int64_t mmz;           // unit : 0.001 [m]
...
private:
    ...
    void processPacket(uint8_t* buffer)
    {
        ...
        pose_msg.pose.position.x = data.mmx * 0.001;
        pose_msg.pose.position.y = data.mmy * 0.001;
        pose_msg.pose.position.z = data.mmz * 0.001;
